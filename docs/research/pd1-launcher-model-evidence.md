...
|---|---|---|
| `PD-LAUNCH-001` | Ordered application roots follow XDG data-directory precedence. Discovery derives Desktop Entry file identifiers, applies higher-root and tombstone shadowing, parses `Type=Application`, locale, visibility, `TryExec`, and execution metadata, and publishes only visible and eligible indices. | Icon-theme lookup, filesystem monitoring, and a production refresh owner remain later work. |
...
| `PD-LAUNCH-003` | `Exec` is parsed and expanded into bounded argument vectors. `%k` receives the exact validated discovered desktop-file location without exposing that path to QML. Actual and terminal executables are resolved from explicit inputs, launch plans contain exact argv, working directory, and environment values, and the detached boundary calls `execve()` directly from a dedicated single-flight worker. | D-Bus activation and desktop actions require separate paths. |
| `PD-LAUNCH-005` | One token-driven launcher surface supports deterministic search/result Tab, Backtab, Up, Down, Enter, and Escape handling; focus follows retained presentation identity across reorder and moves to a deterministic neighbor or search after removal. Controls expose accessible roles, names, descriptions, focus state, and minimum targets. Installed AT-SPI replay covers the integrated launcher surface. | Global shortcut policy and focus transfer across future multi-output shell surfaces remain later integration work. |
...
| `PD-LAUNCH-008` | Search snapshots distinguish `loading`, `results`, `emptyCatalog`, `noResults`, and `error`, carry catalog and request generations for stale-publication rejection, and drive explicit literal-text visual and accessible state labels. Installed AT-SPI replay covers these integrated state labels. | Screen-reader behavior with arbitrary localized application content remains later runtime work. |
| `PD-PERF-006` | Discovery, catalog construction, and search are caller-driven operations with explicit per-call work budgets and cancellation. The shell controller schedules this work on one replaceable worker, rejects stale completions, and uses a separate launch worker. | Filesystem monitoring and measured installed launcher discovery and search responsiveness remain later work. |

`PD-LAUNCH-004` and `PD-LAUNCH-006` are not claimed by this
...
storage, or clearing requirements.
