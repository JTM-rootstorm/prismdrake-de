...
| `PD-LAUNCH-005` | One token-driven launcher surface supports deterministic search/result Tab, Backtab, Up, Down, Enter, and Escape handling; focus follows retained presentation identity across reorder and moves to a deterministic neighbor or search after removal. Controls expose accessible roles, names, descriptions, focus state, and minimum targets. | Installed-session assistive-technology behavior and focus transfer to and from the complete shell still require runtime evidence. |
