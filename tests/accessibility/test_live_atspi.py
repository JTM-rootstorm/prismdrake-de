...
class AccessibilityEvidenceContractTests(unittest.TestCase):
    ...
    def test_selects_exact_owned_panel_for_embedded_accessible_surface(self) -> None:
        node = mock.Mock()
        node.get_process_id.return_value = 42
        node.get_state_set.return_value.contains.return_value = True
        node.get_component_iface.return_value.get_extents.return_value = SimpleNamespace(
        ...
        panel_inventory = SimpleNamespace(returncode=0, stdout="11\n")
        with mock.patch.object(
            live_atspi,
            "_run_checked",
            side_effect=[owner, absent, absent, panel_inventory, panel_inventory],
        ) as run_checked:
            self.assertEqual(
                live_atspi._embedded_panel_for_accessible(
                    atspi, Path("/usr/bin/xdotool"), node, "11", 42
                ),
                "11",
            )
        self.assertEqual(
            run_checked.call_args_list,
            [
                mock.call(["/usr/bin/xdotool", "getwindowpid", "11"]),
                mock.call(
                    [
                        "/usr/bin/xdotool",
                        "search",
    ...
    def test_embedded_surface_rejects_unfocused_foreign_or_invalid_accessible(self) -> None:
        atspi = SimpleNamespace(
            CoordType=SimpleNamespace(SCREEN=1),
            StateType=SimpleNamespace(FOCUSED=2),
        )
        ...
        with mock.patch.object(live_atspi, "_run_checked") as run_checked:
            self.assertIsNone(
                live_atspi._embedded_panel_for_accessible(atspi, xdotool, node, "11", 42)
            )
        run_checked.assert_not_called()
        ...
        component = mock.Mock()
        component.get_extents.return_value = SimpleNamespace(x=20, y=70, width=0, height=48)
    ...
    def test_embedded_surface_rejects_foreign_ambiguous_or_malformed_x11_state(self) -> None:
        node = mock.Mock()
        node.get_process_id.return_value = 42
        node.get_state_set.return_value.contains.return_value = True
        node.get_component_iface.return_value.get_extents.return_value = SimpleNamespace(
        ...
        foreign_owner = SimpleNamespace(returncode=0, stdout="41\n")
        absent = SimpleNamespace(returncode=1, stdout="")
        malformed_absent = SimpleNamespace(returncode=2, stdout="")
        present = SimpleNamespace(returncode=0, stdout="17\n")
        panel_inventory = SimpleNamespace(returncode=0, stdout="11\n")
        ambiguous_inventory = SimpleNamespace(returncode=0, stdout="11\n17\n")
        malformed_inventory = SimpleNamespace(returncode=0, stdout="private-window\n")

        scenarios = (
            [foreign_owner],
            [owner, present],
            [owner, malformed_absent],
            [owner, absent, present],
            [owner, absent, malformed_absent],
            [owner, absent, absent, ambiguous_inventory],
            [owner, absent, absent, malformed_inventory],
            [owner, absent, absent, panel_inventory, ambiguous_inventory],
            [owner, absent, absent, panel_inventory, malformed_inventory],
        )
